...
export class SettingsScene extends Phaser.Scene {
  ...
  create() {
    ...
    headerDiv.lineBetween(0, 68, 390, 68);

    // ── Row 1 — SIGN IN / SIGN UP (y = 115) ──────────────────────────────
    const signInBtn  = this.add.rectangle(130, 115, 110, 44, t.btnBg).setInteractive({ useHandCursor: true });
    this.add.text(130, 115, 'SIGN IN', {
      fontSize: '13px', color: t.btnText, fontFamily: 'Arial, sans-serif', letterSpacing: 2,
    }).setOrigin(0.5);

    const signUpBtn = this.add.rectangle(260, 115, 110, 44, t.btnBg).setInteractive({ useHandCursor: true });
    this.add.text(260, 115, 'SIGN UP', {
      fontSize: '13px', color: t.btnText, fontFamily: 'Arial, sans-serif', letterSpacing: 2,
    }).setOrigin(0.5);
    ...
    rowDiv1.lineBetween(0, 150, 390, 150);
    ...
    const valueText = this.add.text(366, 175, GameSettings.sensitivity.toFixed(2), {
    ...
    this.add.text(195, 240, '1.0', {
      fontSize: '11px', color: t.dimColor, fontFamily: 'Arial, sans-serif',
    }).setOrigin(0.5, 0);
    this.add.text(360, 240, '1.5', {
      fontSize: '11px', color: t.dimColor, fontFamily: 'Arial, sans-serif',
    }).setOrigin(0.5, 0);

    const rowDiv2 = this.add.graphics();
    rowDiv2.lineStyle(1, t.borderColor, 0.25);
    rowDiv2.lineBetween(0, 260, 390, 260);
    ...
    this.makeToggle(335, 405, 'vibration', true);

    const rowDiv4 = this.add.graphics();
    rowDiv4.lineStyle(1, t.borderColor, 0.25);
    rowDiv4.lineBetween(0, 440, 390, 440);
    ...
    this.makeToggle(335, 475, 'darkMode');

    const rowDiv5 = this.add.graphics();
    rowDiv5.lineStyle(1, t.borderColor, 0.25);
    rowDiv5.lineBetween(0, 510, 390, 510);

    // ── Version ───────────────────────────────────────────────────────────
    this.add.text(195, 804, 'v0.1.0', {
  ...
  private makeSensSlider(valueText: Phaser.GameObjects.Text): void {
    const TRACK_X1 = 30, TRACK_X2 = 360, TRACK_Y = 220;
  ...
  private makeDisabledSlider(trackY: number, thumbX: number): void {
    const TRACK_X1 = 30, TRACK_X2 = 360;
    const thumbFill = GameSettings.darkMode ? 0x4444AA : 0x333322;
    const borderNum  = GameSettings.darkMode ? 0x3A3A5E : 0xCCB89A;
